...
func TestParagraphWithImg(t *testing.T) {
	input := `<p>你好哦哦哦哦哦</p><p><img src="gambar1.jpg" data-id="227"></p>`
	...
	paragraph.Text = "你好哦哦哦哦哦"
	img := p.NewTopicContentApp(contentType.Image(), "")
	img.ImgUrl = "gambar1.jpg"
	img.AttachmentsId = "227"
	final := []TopicContentApp{paragraph, img}
...
func TestParagraphWithEmojiDoorAndBook(t *testing.T) {
	...
	emoji1.EmoId = "[emoji]001[/emoji]"
	emoji2 := p.NewTopicContentApp(contentType.Emoji(), "")
	emoji2.EmoId = "[emoji]002[/emoji]"
	...
	final := []TopicContentApp{paragraph, emoji1, emoji2, door, book}
...
func TestParagraphWithEmojiImgDoorAndBook(t *testing.T) {
	...
	paragraph.Text = "你好你好"
	emoji1 := p.NewTopicContentApp(contentType.Emoji(), "")
	emoji1.EmoId = "[emoji]005[/emoji]"
	emoji2 := p.NewTopicContentApp(contentType.Emoji(), "")
	emoji2.EmoId = "[emoji]005[/emoji]"
	img := p.NewTopicContentApp(contentType.Image(), "")
	...
	book := p.NewTopicContentApp(contentType.Book(), "")
	book.BookId = "[book]256[/book]"
	final := []TopicContentApp{paragraph, emoji1, emoji2, img, door, book}
...
func TestParagraphWithEmojiImgVidAudioDoorAndBook(t *testing.T) {
	input := `<html><head></head><body><p>你好[emoji]002[/emoji][emoji]003[/emoji]<p><br/></p></p>
			...
			<video src="https://hjcaecf.top/api/m3u8/video/20230805/d24565e283d772d00336028855673023/95nFNeZITs_i99bb47469a4342cbb64cb91d27671164.m3u8?Expires=1694579920&Signature=eUJBCyoRKkeGRcEsbFx5jm8warPb3nfnueNXClgZkZTUPZoL2JoVWCMEnz5ivoi~fbWmBKUIZqvcTTjCRm01pvXeN8HphUeYHumP6ehzVwLwdcMK36kI9QBCo6TFgnk8AeHdfnxPURS~QbzeDpS4zpqBWZ6CopeLAxEQtcu99kWT8yLGFyeuBADF399YI0rpsvFE9jR79UDV36KBNayeOdd1X1YSuKZ47u76EOTYC8KPAIK1x31mrt3m2Z54wnEX9D~UPwbc5kaBhNTDtEhu12BAUf7r2s6g9al-tBhLrYowCsxD7uPVKX3NmQpbcFsTVQTED5xeohGFjn2XI-hjqg__&Key-Pair-Id=K3HEWB9S9B6R6N" data-id="95"></video>
			<audio src="https://res.hjcaecf.top/hjstore/audio/20230720/cf53d6f6d358a9f3e8c827bf2086b340.mp3" controls="true" controlslist="nodownload" id="showaudio" data-id="2"></audio>
			[door]444327[/door][book]256[/book]<p><p><br/></p><p><br/></p></p></body></html>`
	got := startParsing(input)
	if len(got) != 8 {
	...
	book := p.NewTopicContentApp(contentType.Book(), "")
	book.BookId = "[book]256[/book]"
	final := []TopicContentApp{paragraph, emoji1, emoji2, img, vid, audio, door, book}
